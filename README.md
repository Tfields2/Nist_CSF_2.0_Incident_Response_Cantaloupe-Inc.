# NIST CSF 2.0 Incident Response Cantaloupe Inc.

## Overview
This project involved the develpment and documentaion of an incident response program aligned with the NIST CSF 2.0 for Cantaloupe Inc. services environment. The work focused on establishing a structured, compliance aware approach to incident identification, containment, remediation, recovery, and continuos improvement, and was formally presented to a cybersecurity board for evaluation. 

## Scope
- Financial services environment
- Enterprise incident response governance
- Aligment with NIST 2.0 Respond and Recover functions
- Integration of legal, compliance, and business continuity considerations

## Framework & Standards
- NIST Cybersecurity Framework CSF 2.0
- Incident Command Structure
- Financial sector regulatory consideration (e.g., SEC disclosure, GLBA concepts)

## Repository Contents
- [LoanDepot Challenge Prompts](https://github.com/Tfields2/Nist_CSF_2.0_Incident_Response_Cantaloupe-Inc./blob/c602c249c30183e8292e198e2dc21a424748fc62/Loandepot_Challenge_Prompt.pdf)
- [Formal Incident Response Documentation](https://github.com/Tfields2/Nist_CSF_2.0_Incident_Response_Cantaloupe-Inc./blob/5035d930e3192260171caa4c34644f31fdbc7599/Loandepot_ResponsePlan.pdf)
- [LoanDepot Links](https://github.com/Tfields2/Nist_CSF_2.0_Incident_Response_Cantaloupe-Inc./blob/c602c249c30183e8292e198e2dc21a424748fc62/Loan%20Depot%20Links.pdf)

## Key Contributions
- Defined roles, escalation paths, and communication workflows
- Developed scenerio-based response playbooks
- Mapped response activities to NIST CSF 2.0 categories and outcomes

## Intended Audience
- SOC Analysts
- Incident Responders
- GRC and Compliance Teams
- Academic and portfolio Review


## Disclaimer
This material is intended solely for educational and portfolio demonstration purposes. Any resemblance to real organizations or incidents is generalized and non-attributable. The content does not disclose sensitive operational details, confidential information, or proprietary processes.

